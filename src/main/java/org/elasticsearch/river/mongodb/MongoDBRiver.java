            ...
            try {
                ...
                if(children != null) {
                    if (OPLOG_INSERT_OPERATION.equals(operation) || OPLOG_UPDATE_OPERATION.equals(operation)) {
                        ...
                        // For update operation we should delete current indexed children data by parent id (objectId)
                        ...
                        if(!(data.get(children) instanceof Collection)) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("Children interface should be java.util.Collection and was {}", data.get(children).getClass().toString());
                            }

                            return lastTimestamp;
                        }

                        Collection childrenData = (Collection) data.get(children);
